...
A continuación tenemos que crear un archivo con ambos ficheros combinados, mediante el comando ```unshadow```, al que le daremos como nombre ```unshadow```:

```
vagrant@ubuntu2004:~$ unshadow passwd shadow > unshadow
```

Su contenido es el siguiente:
```
vagrant@ubuntu2004:~$ cat unshadow.txt 
...
Teniendo el archivo ```unshadow``` con el formato preparado, podremos lanzar JtR mediante el siguiente comando:

```
john unshadow
```
 Podemos indicarle el fichero con la lista de contraseñas con la opción --wordlist
