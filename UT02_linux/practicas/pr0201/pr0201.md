...
- rwxr-xr-x : ```chmod u+x file && chmod g+x file && chmod o+x file```
- rwxr-xr-x : ```chmod u+x file && chmod g+x file && chmod o+x file```
- r-x--x--x : ```chmod u=rx file && chmod g=x file && chmod o=x file```
...
- rw-r----- : ```chmod o-r file```
- rwx--x--x : ```chmod u+x file && chmod g=x file && o=x file```
...
aya:$y$j9T$Gs0y3myX9jNbJBTsMSzJL.$i0cqYY45naZWn6MKGegyl7PVKlTDUOoVus4TJ6W.L6B:19997:0:99999:7:::
```
...
Mi sistema utiliza el **yescrypt**, ya que los hash de las contraseñas comienzan por ```$y$```

5. ¿Para qué sirve el segundo campo del hash llamada **sal**?

La **sal**, o el **salado**, es una técnica que agrega un **valor aleatorio** a la contraseña antes del hash. La sal se almacena junto a la contraseña cifrada en ```/etc/shadow```. Ayuda a proteger contra ataques de diccionario y rainbow.
...
1. Verificar que el hash del fichero ```/etc/shadow``` es el hash de la contraseña ```asir1```

Para realizar esto, primero instalaré el paquete ```whois``` con ```sudo apt-get install whois```

El comando ```mkpasswd``` viene incluido en este paquete y lo usaremos para obtener el hash de nuestra contraseña, introduciendo el comando para el sistema **yescrypt** seguido de la contraseña ```asir1```:

```
vagrant@ubuntu2204:~$ mkpasswd -m yescrypt
Password:
$y$j9T$qUveVf8Gjn23OrwfU3qEn0$HBciZJT7nrksuAVLpjcU2ekdevlw7amPj1PCmXsCYb0
```

Si observamos el hash del fichero ```/etc/shadow``` para el usuario que creamos, veremos que coincide:

```
vagrant@ubuntu2204:~$ sudo tail /etc/shadow
aya:$y$j9T$Gs0y3myX9jNbJBTsMSzJL.$i0cqYY45naZWn6MKGegyl7PVKlTDUOoVus4TJ6W.L6B:19997:0:99999:7:::
```
